function [params] = SC(params)
    ...
    params.gray = [103    87   125]; % The gray of the background. Very important!
    ...
    params.fixUpperRightRect = CenterRectOnPointd(params.baseRect*1,xCenterExp+params.horzDist,yCenterExp-params.vertDist); % Create upper FixRect choice rectangle (Index 1, clockwise)
    params.fixLowerRightRect = CenterRectOnPointd(params.baseRect*1,xCenterExp+params.horzDist,yCenterExp+params.vertDist); % Create lower right choice rectangle (Index 2, clockwise)
    params.fixLowerLeftRect = CenterRectOnPointd(params.baseRect*1,xCenterExp-params.horzDist,yCenterExp+params.vertDist); % Create lower left choice rectangle (Index 3, clockwise)
    params.fixUpperLeftRect = CenterRectOnPointd(params.baseRect*1,xCenterExp-params.horzDist,yCenterExp-params.vertDist); % Create upper left choice rectangle (Index 4, clockwise)
    ...
    achromList = [1:length(achromTex)]; % Give a list of numbers from one to the number of achromatic textures
    circleList = [1:length(circleTex)]; % Give a list of numbers from one to the number of circle textures
    bwList = [1:length(BWTex)]; % Give a list of numbers from one to the number of black and white textures
    ...
    if sum(params.blockorder==params.achromCase)>0 % Are there achromatic shapes
        ...
        % Begin Stimulus
            ...
            if keyIsDown == 1
                ...
                elseif keyCode(KbName('w')) && params.fixPix > params.pixPerAngle/2 % Increase params.fixation circle
                    ...
                    params.baseFixRect = [0 0 params.fixPix params.fixPix]; % Size of the params.fixation circle
                    params.fixRect = CenterRectOnPointd(params.baseFixRect, xCenterExp, yCenterExp); % We center the params.fixation rectangle on the center of the screen
                    params.fixUpperRightRect = CenterRectOnPointd(params.baseRect*2,xCenterExp+params.horzDist,yCenterExp+params.vertDist); % Create upper right choice rectangle (Index 1, clockwise)
                    params.fixLowerRightRect = CenterRectOnPointd(params.baseRect*2,xCenterExp+params.horzDist,yCenterExp-params.vertDist); % Create lower right choice rectangle (Index 2, clockwise)
                    params.fixLowerLeftRect = CenterRectOnPointd(params.baseRect*2,xCenterExp-params.horzDist,yCenterExp-params.vertDist); % Create lower left choice rectangle (Index 3, clockwise)
                    params.fixUpperLeftRect = CenterRectOnPointd(params.baseRect*2,xCenterExp-params.horzDist,yCenterExp+params.vertDist); % Create upper left choice rectangle (Index 4, clockwise)
                    params.fixRects = {params.fixUpperRightRect,params.fixLowerRightRect,params.fixLowerLeftRect,params.fixUpperLeftRect}; % Cell array of params.fixation choice rects, this needs to be repeated later in the main code for updating
