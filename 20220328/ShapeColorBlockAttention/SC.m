function SC(subject, counterbalance_indx, run)
    ...
    choiceDistAngle = 5; % The presented choices will be seperated by 10 degrees of visual angle
    ...
    % Exact Duration
    runDur = TR*blocklength*length(blockorder)+30; % Calculating this to compare to exact duration
    ...
    rightFixRect = CenterRectOnPointd(baseFixRect*1.75, xCenterExp+0.5*distPix, yCenterExp); % Area of fixation that can be looked at for choice on the right
    leftFixRect = CenterRectOnPointd(baseFixRect*1.75, xCenterExp-0.5*distPix, yCenterExp); % Area of fixation that can be looked at for choiceo n the left
