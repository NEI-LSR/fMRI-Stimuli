...
calibName = '27-Jun-2023_Lab_Monitor'; % What is the name of the files that stores the calibration information?
calibpath = fullfile(measuredir,calibName); % Where is this path?
extension = 'DKL670TrainingLAB';
...
LMSgray = spectra2LMS(LumValues.white(8).Spectrum,'LMS'); % Load LMS value of the gray you measured
graypointRGB = repmat(LumValues.white(8).gunValue,1,3); % Get RGB value
...
scalingFs = repmat([0.3 0.6 0.9], 1, 8); % How far along the gamut of the direction with the least gamut range do you want to extend?
angles = [0 0 0 45 45 45 90 90 90 135 135 135 180 180 180 225 225 225 270 270 270 315 315 315]; % What angles in DKL space do you want to compute your colors around
lumAngles = repmat([0 0 0 0], 1, 6); % What angle along the LMS axis do you want to calculate around?
...
legend('Equiluminant Gray','Target Values','Monitor Gamut');
