function FiveDot
    ...
    ttlChannel = 8;
    numAverageCoords = 5; % Number of coordinates to average ove
    rewardDur = 0.03; % seconds
    rewardFreq = 0.5; % Hz
    curRewardFreq = rewardFreq; % Hz
    maxRewardFreq = 1.25; % Hz
    rewardIncStep = 0.05; % Hz
    rewardBreakTime = 0.5; % Seconds
    rewardWait = 1/rewardFreq; % seconds
    rewardDurChange = 0.001; 
    newRewardRate = rewardWait;
    rewardWaitChange = 0.01;
    maxChange = 0.2; % How much does it change
    rewardCalcDur = 30; % Number of seconds fixation is calculated over 
    ...
    % Prep mvie info
    movieName = 'our_planet.mp4' %'sherlock_seg1.mp4';
    ...
    rawEyePosition = NaN(fps*exactDur,2);
    ...
    try
        ...
        for frameIdx = 1:fps*exactDur
            ...
            [rawEyePosition(frameIdx,1), rawEyePosition(frameIdx,2)] = eyeTrack(xChannel, yChannel, xGain, yGain, xOffset, yOffset);
            if frameIdx > numAverageCoords
                eyePosition(frameIdx,1) = mean(rawEyePosition((frameIdx-numAverageCoords):frameIdx,1),"omitnan");
                eyePosition(frameIdx,2) = mean(rawEyePosition((frameIdx-numAverageCoords):frameIdx,2),"omitnan");
            else
                eyePosition(frameIdx,1) = mean(rawEyePosition(1:frameIdx,1),"omitnan");
                eyePosition(frameIdx,2) = mean(rawEyePosition(1:frameIdx,2),"omitnan");  
            end
            fixation(frameIdx,1) = isInCircle(eyePosition(frameIdx,1),eyePosition(frameIdx,2),fixRect);
            % Process Keys
            if keyCode(KbName('r')) % Recenter
                xOffset = xOffset + rawEyePosition(frameIdx,1)-xCenterExp;
                yOffset = yOffset + rawEyePosition(frameIdx,2)-yCenterExp;
            ...
            elseif keyCode(KbName('3#')) % Juice
                maxRewardFreq = maxRewardFreq + rewardWaitChange;
            ...
            text = ['xGain: ', num2str(xGain), newline,...
                ...
                'Reward Wait Time: ' num2str(rewardWait),newline,...
                'Reward Frequency: ' num2str(rewardFreq),newline,...
                'Max Reward Frequency (+1/-2) :' num2str(maxRewardFreq)];
            ...
            if frameIdx > fps*rewardWait
                [juiceOn, juiceDistTime,timeSinceLastJuice,rewardWait,curRewardFreq] = juiceCheck(juiceOn, frameIdx,fps,rewardWait,fixation,juiceDistTime, rewardPerf,rewardDur,timeSinceLastJuice,curRewardFreq,maxRewardFreq,rewardIncStep);
            end
            if frameIdx > rewardBreakTime*fps
                if all(fixation(frameIdx-floor(rewardBreakTime*fps):frameIdx) == 0)
                    curRewardFreq = rewardFreq;
                    rewardWait = 1/curRewardFreq;
                end
            end
            if quitNow == true
                sca;
    ...
    function [juiceOn, juiceDistTime,timeSinceLastJuice,rewardWait,curRewardFreq] = juiceCheck(juiceOn, frameIdx,fps,rewardWait,fixation,juiceDistTime, rewardPerf,rewardDur,timeSinceLastJuice,curRewardFreq,maxRewardFreq,rewardIncStep)
        ...
        if juiceOn == false && timeSinceLastJuice > rewardWait && sum(fixation(frameIdx-fps*rewardWait+1:frameIdx),"all") > rewardPerf*fps*rewardWait
            juiceOn = true;
            if curRewardFreq < maxRewardFreq
                curRewardFreq = curRewardFreq + rewardIncStep; % increase reward frequency
            end
            rewardWait = 1/curRewardFreq; % Translate into seconds
        end
