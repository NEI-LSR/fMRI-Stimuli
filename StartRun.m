function [params] = StartRun(params, window, fixGridTex, movieTex, varargin)
    
    if length(varargin) == 1 % Bring in window2 if specified
        window2 = varargin{1};
    end
    ...
    elseif params.system.screens == 2
        ...
        startTime = tic;
        Screen('FillRect', window, params.display.grayBackground);
        Screen('FillRect', window2, params.display.grayBackground);

        [~, time2flip] = Screen('Flip', window);
        [~, time2flip2] = Screen('Flip', window2);
        flips = 0:params.display.ifi:params.run.exactDuration + params.display.ifi
        flips = flips + time2flip
        ...
        for frameIdx = 1:params.run.exactDuration * params.display.fps
            ...
            %time2flip2 = time2flip2 + params.display.ifi*0.99;
            
            % Drawing fixation grid on framebuffer
            Screen('DrawTexture', window2, fixGridTex, [], params.display.expRect+[params.run.fixation.dotOffset,params.run.fixation.dotOffset]);
            if params.run.fixation.isGridOn
                Screen('DrawTexture', window, fixGridTex, [], params.display.monkRect+[params.run.fixation.dotOffset,params.run.fixation.dotOffset]);
            end
            ...
            % Drawing fixation dot on framebuffer
            if params.run.fixation.isDotOn
                Screen('DrawDots', window2, [0 0], params.run.fixation.dotSize, params.run.fixation.dotColor, params.display.expRectCenter+params.run.fixation.dotOffset, 1);
                Screen('DrawDots', window, [0 0], params.run.fixation.dotSize, params.run.fixation.dotColor, params.display.monkRectCenter+params.run.fixation.dotOffset, 1);
            end
            ...
            if ~isnan(movieTex(frameIdx))
                [text2draw, margin, textSize] = FormatText2draw;
                DrawFormattedText(window2, text2draw, margin, params.display.expWindowRect(4)-textSize-margin, 1, [], [], [], [], [], params.display.expWindowRect);
            end

            % Drawing experimenter overlay on framebuffer (fixation window circle)
            Screen('FrameOval', window2, params.run.fixation.windowColor(params.run.fixation.isInWindow+1,:), OffsetRect(params.run.fixation.windowRect,-params.display.expWindowRect(3),0), 3);

            % Drawing experimenter overlay on framebuffer (monkey's gaze position)
            if params.system.debug == false
                if IsInRect(params.run.fixation.coordinates(end,1), params.run.fixation.coordinates(end,2), params.display.monkWindowRect)
                    gazeRect = round([params.run.fixation.coordinates(end,1) params.run.fixation.coordinates(end,2) params.run.fixation.coordinates(end,1) params.run.fixation.coordinates(end,2)]) + [-5 -5 5 5];
                    Screen('FillOval', window2, params.run.fixation.windowColor(params.run.fixation.isInWindow+1,:), OffsetRect(gazeRect,-params.display.expWindowRect(3),0));
                end
            end

            [~,params.run.log(frameIdx,1)] = Screen('Flip', window, flips(frameIdx));
            [~,params.run.log(frameIdx,2)] = Screen('Flip', window2, flips(frameIdx));
            ...
            % Logging data
            params.run.log(frameIdx,3:end) = [params.run.reward.count params.run.reward.frequency params.run.fixation.windowSize params.run.fixation.breakTolerance params.run.fixation.isGridOn params.run.fixation.isDotOn params.run.fixation.isInWindow];
        ...
        Screen('FillRect', window, params.display.blackBackground);
        Screen('Flip', window);
        Screen('FillRect', window2, params.display.blackBackground);
        Screen('Flip', window2);
        Priority(0);
    ...
    function CheckKeyboard
        ...
        if keyIsDown && secs > params.key.lastPress + params.key.minPressDuration
            ...
            elseif keyCode(params.key.dot2down)
                ...
                params.run.fixation.windowRect = CenterRect([0, 0, params.run.fixation.windowSize.*params.display.pixPerDeg], params.display.monkWindowRect+[params.run.fixation.dotOffset params.run.fixation.dotOffset]);
            elseif keyCode(params.key.quit)
                sca;
            end
        end
