function LUT = makelutNEW(bitdepth, calibFileRoot)


NAME  = who('-file', strcat(calibFileRoot, '.mat')); 
DAT   = load(strcat(calibFileRoot, '.mat')); 
nbValues = 2^bitdepth;
...
save([calibFileRoot, '_LUT.mat'], 'LUT')
save([calibFileRoot, '_GammaTable.mat'], 'GammaTable')
save([calibFileRoot, '_GammaTableCorrected.mat'], 'GammaTableCorrected')

end 


function [xlut, gammaTable, gammaTableCorrected] = makeLUTperGun(xdata, ydata, col, varargin)
global calibFileRoot
...
% ylim([-0.05, 1.05])
% axis square
% %saveas(fig, sprintf('%s_%sGunFit.eps',calibFileRoot, col),'epsc')
% saveas(fig, sprintf('%s_%sGunFit.png',calibFileRoot, col),'png')
