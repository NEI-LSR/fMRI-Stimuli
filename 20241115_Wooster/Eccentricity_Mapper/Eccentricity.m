function Eccentricity(subject, counterbalance_indx, run, ima)
    % Eccentricity 1.0
    ...
    dataSaveFile = ['Data/' subject '_' num2str(run) '_IMA' num2str(ima) '_Data.mat']; % File to save both run data and eye data
    ...
    exactDur = 480; % Specify this to check
    ...
    % LM_..., SLM_...
    color_conds = {'BW'}; %{'BW','LM','S'};
    ecc_conds = {'DVA1','DVA3','DVA7', 'DVA14'}; %{'Foveal','Middle','Peripheral'};
    ...
    pixPerAngle = (1920/(rad2deg(atan(1/57))*38.2)); %1080/(atan(28/57)*180/pi); % Number of pixels per degree of visual angle
    ...
    for i = 1:length(blockorder)
        switch blockorder(i)
            ...
            case 4 % 14
                frames = (1:framesPerBlock)+(i-1)*framesPerBlock;
                texture(frames) = repmat(repelem(baseTex(8:9),framesPerStim),1,framesPerBlock/(framesPerStim*stimPer)); %repmat(baseTex(1),1,framesPerBlock);
            %case 6 % LM Foveal
            %    frames = (1:framesPerBlock)+(i-1)*framesPerBlock;
