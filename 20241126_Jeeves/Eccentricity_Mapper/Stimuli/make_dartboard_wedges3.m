...
mask_inner_radius_deg = 6.5;
mask_outer_radius_deg = 10.0;
...
hb_deg_per_pixel = 1/(1920/(rad2deg(atan(1/57))*38.2)); %1/50; %(atan(28 / 57.0) * 180 / pi)/ 1080;
		...
		for j_wedge = 1:NumWedges
			...
			if ~isempty(img_idx),
				switch color_type
                    ...
                    case 'color-alt'
                        if odd_ring == 0,
                            sector_col = alt_cols{odd_wedge+1};
                        else
                            sector_col = alt_cols{abs(odd_wedge-2)};
                        end
                        if odd_im == 0,
                            if odd_ring == 0,
                                sector_col =  alt_cols{abs(odd_wedge-2)};
