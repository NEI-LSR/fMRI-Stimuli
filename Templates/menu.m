...
params.ttChannel = 8; % Where the TTL channel inputs to the DAQ
...
params.blockorders = repmat(williams(numconds),1,params.blockrepeats); % Generated using Williams counterbalancing (see williams.m) and repeated the matrix by 3 in its length
...
while true % Now we run a while loop to actually display the menu
    prompt = ['Menu',newline,...
        ...
        'Gray Duration (TRs): ' num2str(params.grayDur),...
        ...
        'We have completed ' num2str(params.runNum) ' this session.',newline,...
        newline,...
        'Enter 1 to begin fixation',newline,...
        'Enter 2 to generate run orders for ' params.experiment,newline,...
        'Enter 3 to begin ' params.experiment, newline,...
        'Enter P to quit', newline,...];
