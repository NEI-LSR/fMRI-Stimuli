function ALZ_dyLocX_standard_BW_NIF(subj_id, counterbalance_idx, acquisition_num)
...
%    [window, rect] = Screen('OpenWindow', screenNumber,[0,0,0],[2320,0,3420,800]);
    %[window, rect] = Screen('OpenWindow', screenNumber,[0,0,0],[1920,0,3840,1080]);
...
%% wait for MRI trigger
    currDir = pwd;
    cd(rootDir)
    signal_init = 0;%fnDAQNI('GetAnalog',7);
    skip=1;
    while 1
        ...
        signal_current = 0;%fnDAQNI('GetAnalog',7);
...
function eyePos = getEyePos(gain,Vcent,center)
    global channels
    Vraw = [0,0];%fnDAQNI('GetAnalog',channels);
...
function recenter()
    ...
    Vcent = [0,0];%fnDAQNI('GetAnalog',channels);
end
