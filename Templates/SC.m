function [params] = SC(params)
    ...
    params.dataSaveFile = fullfile(params.dataDir,[params.subject '_' num2str(params.runNum) '_' date_time '_Data.mat']); % File to save both run data and eye data
    params.movSaveFile = fullfile(params.dataDir,[params.subject '_' num2str(params.runNum) '_' date_time '_Movie.mov']); % Create Movie Filename
    params.DMSaveFile = fullfile(params.dataDir,[params.subject '_' num2str(params.runNum) '_' date_time '_DM.txt']); % Create Design Matrix Filename
    ...
    achromOrder = randsample(length(achromTex),params.numblocks); % Order by which achromatic shapes are presented

    while true % run a while loop to make sure that no associated colors and shapes are adjacent
        circleOrder = randsample(length(circleTex),params.numblocks); % Order by which colors are presented
        bwOrder = randsample(length(BWTex),params.numblocks); % Order by which black and white shapes are presented
        ...
        testblockorder(params.blockorder==params.bwCase)==beOrder; % Fill in black and white indices
    ...
    orderNames = NaN(size(params.blockorder)); % Initialize ordernames
    orderNames(params.blockorder==params.colorCase)==circleOrderNames; % Add colored circles to the ordernames
    orderNames(params.blockorder==params.bwCase)==bwOrderNames; % Add chromatic shapes to the ordernames
    orderNames(params.blockorder==params.achrom)==achromOrderNames; % Add achromatic shapes to the ordernames
    ...
    try
        ...
        % Begin Stimulus
           while true
            ...
            if blockTime >=params.blocklength*TR | frameIdx == 1;
                ...
                if blockIndx <= length(blockorder)
                    switch blockorder(blockIndx) % Setting all the relevant texture info. Really should consolidate this with the code below
                        case achromCase % ACh
                            ...
                            objectName = params.achrom(achromIndx);
                        ...
                        case colorCase % Colored Cirles
                            ...
                            objectName = params.colors(circleIndx);
                        ...
                        case bwCase % Black and White Color Associated Shapes
                            ...
                            objectName = params.chrom(chromIndx);
                            bwIndx = bwIndx+1;
                ...
                if isgray==false && params.choiceSectionDur > 0 % Add one to choice index
            ...
            if isgray == true
                choice = 'None';
            elseif params.probeIndices(blockIndx) == 1 && params.choiceSectionDur > 0
            ...
            elseif keyCode(KbName('z')) % Increase Choice Reward Dur
                choiceRewardDur = choiceRewardDur + params.rewardKeyChange;
                keyIsDown = 0;
            elseif keyCode(KbName('x')) && choiceRewardDur > params.rewardKeyChange % Decrease Choice Reward Dur
                choiceRewardDur = choiceRewardDur - params.rewardKeyChange;
                keyIsDown = 0;
            ...
            infotext = ['Time Elapsed: ', num2str(toc), '/', num2str(params.runDur), newline,...
                ...
                'Object Name: ' objectName,newline,...
            ...
            elseif blockTime >= stimDur*TR && blockTime < (stimDur+grayDur)*TR % Inter event interval
                ...
                % Draw eyetrace on framebuffer
                Screen('DrawDots',expWindow, eyePosition(frameIdx,:)',5);
            elseif blockTime >= (stimDur+grayDur)*TR && params.choiceSectionDur > 0 && params.probeIndices(blockIndx) == 1 % Choice interval
                ...
                elseif isgray == false
                    ...
                    if sideChoices(choiceIndx) == 0
                        if correctSideChoices(choiceIndx) == 1 % Left side
                            ...
                            if sideChoices(choiceIndx) == 0 && sum(leftFixation(frameIdx-(choiceDur*params.FPS+1):frameIdx),'all','omitnan') == choiceDur*params.FPS
                            ...
                            elseif sideChoices(choiceIndx) == 0 && sum(rightFixation(frameIdx-(choiceDur*params.FPS+1):frameIdx),'all','omitnan') == choiceDur*params.FPS
            ...
            % ALSO SAVE
            if rem(frameIdx,params.FPS/movieFPS)==1
    ...
    function [xPos, yPos] = eyeTrack(params.xChannel, params.yChannel, params.xGain, params.yGain, params.xOffset, params.yOffset)
        coords = DAQ('GetAnalog',[params.xChannel params.yChannel]);
        xPos = (coords(1)*params.xGain)-params.xOffset;
        yPos = (coords(2)*params.yGain)-params.yOffset;
    end
