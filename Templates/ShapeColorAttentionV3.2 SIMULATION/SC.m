function [params] = SC(params)
    ...
    Screen('Preference', 'SuppressAllWarnings', 1); % Supress all warnings
     ...
     [expWindow, expRect] = Screen('OpenWindow', params.expscreen, params.gray, [0,0,30,30]); % Open experimenter window
    ...
    try
        ...
        % Wait for TTL
        baselineVoltage = 5; % Get the baseline voltage
        while true
            [keyIsDown,secs,keyCode] = KbCheck; % Get keyboard inputs
            ttlVolt = 5; % Get current voltage
            if true % Always true for simulation purposes
                break; % Begin
