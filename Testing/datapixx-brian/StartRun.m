function [params] = StartRun(params, window, fixGridTex, retinoMovie, maskIdx56)%isStimOn84, scotomaTex)
    ...
    % Beginning run
    for frameIdx = 1:params.run.exactDuration * 15
        params.run.frameIdx = frameIdx;
        time2flip = time2flip + params.display.ifi*4;
        ...
        if maskIdx56(frameIdx)>1
            movieTex = Screen('MakeTexture', window, retinoMovie(:, :, :, frameIdx));
            Screen('DrawTexture', window, movieTex, [], params.display.expRect, [], [], params.run.stimContrast);
            Screen('DrawTexture', window, movieTex, [], params.display.monkRect, [], [], params.run.stimContrast);
            Screen('Close', movieTex);
            %Screen('DrawTexture', window, scotomaTex, [], params.display.expRect);
            %Screen('DrawTexture', window, scotomaTex, [], params.display.monkRect);
        end
