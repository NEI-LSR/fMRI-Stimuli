function [params] = MapPRF()
    ...
    %[scotomaImg, ~, alpha] = imread([params.directory.stimuli '/' 'artificial_scotoma_99_fullscreen.png']);
    %scotomaImg(:,:,4) = alpha;
    %scotomaTex = Screen('MakeTexture', window, scotomaImg);
    %[fixGridTex, retinoTex, motionTex] = MakePTBtextures2(params, window);
    [fixGridTex, retinoMovie] = MakePTBtextures2(params, window);
...
%     load([params.directory.stimuli '/' 'maskIdx.mat'], 'maskIdx');
%     retinoTex = NaN(length(isStimOn84), 1);
%     for idx = 1:length(isStimOn84)
%         if isStimOn84(idx)
...
%     retinoTex    = repelem(retinoTex, params.display.fps/playbackFps);
    ...
    while true
        ...
        elseif selection == 2
            ...
            %params = StartRun(params, window, fixGridTex, retinoMovie, isStimOn84, scotomaTex);
            params = StartRun(params, window, fixGridTex, retinoMovie, maskIdx56);
        ...
        switch type
            ...
            case 'input'
                ...
                inputs = inputdlg({'Subject ID', ...
                                   ...
                                   'Reward frequency increment (Hz)', ...
                                   'Number of samples for eye temporal filter'}, 'Parameters', 1, ...
                                  {subjectID, ...
                                   ...
                                   num2str(params.run.fixation.numSamples)});
                if ~isempty(inputs)
                    ...
                    params.run.fixation.numSamples       = str2double(inputs{11});
                end
        end
